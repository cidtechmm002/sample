...
export function BeforeAfter({ before, after, alt = "", className = "" }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute top-4 left-4 rounded-full bg-[color:var(--dark-forest)] border border-white/10 px-4 py-1.5 text-[10px] tracking-[0.3em] uppercase text-white shadow-lg backdrop-blur-sm">
        Before
      </div>
      <div className="pointer-events-none absolute top-4 right-4 rounded-full bg-[color:var(--gold)] border border-[color:var(--gold-light)]/40 px-4 py-1.5 text-[10px] tracking-[0.3em] uppercase text-[color:var(--dark-forest)] font-semibold shadow-lg">
        After
      </div>
      <div className="absolute inset-y-0 w-px bg-[color:var(--gold)]" style={{ left: `${pos}%` }}>
